...
current word $$x_t$$, $$W_x$$ encodes what features of the present are important to determine the future. 
 ...
 If $$ \beta_W \beta_h$$ is less than one, $$ (\beta_W \beta_h)^{t-k}$$ will tend towards zero, and if $$ (\beta_W \beta_h)$$ is greater than one, 
 than $$ (\beta_W \beta_h)^{t-k}$$ will tend towards and an incredibly large number that we can't compute, and thus our total gradient:
 \begin{equation}
